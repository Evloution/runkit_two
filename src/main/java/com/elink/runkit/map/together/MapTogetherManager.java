package com.elink.runkit.map.together;
...
import com.elink.runkit.util.DisplayUtil;
...
public class MapTogetherManager {
    ...
    private static Map<String, TogMarketBean> togDotInfoMap;// 聚合网点列表
    ...
    private MapTogetherManager(Activity mContext, AMap aMap) {
        ...
        this.aMap = aMap;
        togDotInfoMap = new ConcurrentHashMap<String, TogMarketBean>();
            ...
            public void run() {
                synchronized (lockObject) {
                    ...
                    if (togDotInfoMap != null && togDotInfoMap.size() > 0) {
                        Iterator<Map.Entry<String, TogMarketBean>> cIterator = togDotInfoMap.entrySet().iterator();
                        while (cIterator.hasNext()) {
                            Map.Entry<String, TogMarketBean> togMap = cIterator.next();
    ...
    private void assignSingleCluster(Marker marker) {
        MonitoringPointBean dotInfo = (MonitoringPointBean) marker.getObject();
        LatLng latLng = new LatLng(dotInfo.getLATITUDE(), dotInfo.getLONGITUDE());

        Point point = aMap.getProjection().toScreenLocation(latLng);
        TogMarketBean togMarketBean = getCluster(point);
        if (togMarketBean != null) {
            togMarketBean.addClusterItem(marker);
            // 更新聚合网点个数
            togMarketBean.setDotCount(togMarketBean.getDotCount() + 1);
        } else {
            togMarketBean = new TogMarketBean(point, latLng);
            // 更新聚合网点个数
            togMarketBean.setDotCount(1);
            togMarketBean.addClusterItem(marker);
            togDotInfoMap.put(dotInfo.getIP() + SystemClock.currentThreadTimeMillis(), togMarketBean);
        }
    }

    /**
     * 将聚合网点添加至地图显示
     * @param togMarketBean 需要更新的聚合网点对象
     */
    private void addTogDotInfoToMap(String dotId, TogMarketBean togMarketBean) {
        LatLng latlng = togMarketBean.getCenterLatLng();
        MarkerOptions markerOptions = new MarkerOptions();
        markerOptions.anchor(0.5f, 0.5f).icon(getBitmapDes(togMarketBean)).position(latlng);
     ...
     * 获取每个聚合点的绘制样式
     **/
    private BitmapDescriptor getBitmapDes(final TogMarketBean togMarketBean) {
        TextView textView = new TextView(mContext);
        String tile = String.valueOf(togMarketBean.getDotCount());
        textView.setText(tile + "");
        textView.setGravity(Gravity.CENTER);

        textView.setTextColor(Color.BLACK);
        textView.setTextSize(TypedValue.COMPLEX_UNIT_SP, 15);
        textView.setBackgroundDrawable(getDrawAble(togMarketBean.getDotCount()));
        togMarketBean.setTextView(textView);
     ...
     * @return
     */
    private TogMarketBean getCluster(Point point) {
        Iterator<Map.Entry<String, TogMarketBean>> cIterator = togDotInfoMap.entrySet().iterator();
        while (cIterator.hasNext()) {
            TogMarketBean togMarketBean = cIterator.next().getValue();
            Point poi = togMarketBean.getCenterPoint();
            double distance = getDistanceBetweenTwoPoints(point.x, point.y, poi.x, poi.y);
            if (distance < CLUSTER_SIZE) {
                return togMarketBean;
            }
        }
